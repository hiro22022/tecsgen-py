...
#   tecslib/plugin/MrubyBridgeSignaturePlugin.rb を Python へ移植したものである．
#
...
class MrubyBridgeSignaturePlugin(MrubyBridgeSignaturePluginModule, SignaturePlugin):

    b_no_banner = False  # class-level, separate from MrubyInfoBridgeSignaturePlugin

    def __init__(self, signature, option):
        super().__init__(signature, option)

        if not MrubyBridgeSignaturePlugin.b_no_banner:
            sys.stderr.write(
                "MrubyBridgePlugin: version 2.0.0 (Suitable for mruby above ver 1.2.0).\n"
            )
            MrubyBridgeSignaturePlugin.b_no_banner = True
        ...
        for func_head in signature.get_function_head_array():
            if len(self.includes) > 0:
                if func_head.get_name() in self.includes:
                    dbgPrint("MrubyBridgePlugin: {} INCLUDED\n".format(func_head.get_name()))
                    fh_array.append(func_head)
                else:
                    dbgPrint("MrubyBridgePlugin: {} NOT included\n".format(func_head.get_name()))
            elif len(self.excludes) > 0:
                if func_head.get_name() not in self.excludes:
                    dbgPrint("MrubyBridgePlugin: {} NOT excluded\n".format(func_head.get_name()))
                    fh_array.append(func_head)
                else:
                    dbgPrint("MrubyBridgePlugin: {} EXCLUDED\n".format(func_head.get_name()))
            else:
                fh_array.append(func_head)
        ...
        for fh in fh_array:
            if self.auto_exclude_list.get(fh.get_name()) is None:
                ...
                dbgPrint("MrubyBridgePlugin: auto_exclude {}\n".format(fh.get_name()))

        if len(self.func_head_array) == 0:
            self.cdl_error("MRB1012 '$1' no function remained by exclude",
    ...
    def check_struct_member(self, struct_type, fh):
        ...
        if self.struct_list.get(sttype.get_name()) is None:
            print("  MrubyBridgePlugin: [struct]   {} => [class] TECS::Struct{}\n".format(
    ...
    def register_ptr_type(self, ttype, fh):
        ...
        if MrubyBridgeSignaturePluginModule.ptr_list.get(ptr_celltype_name) is None:
            print("  MrubyBridgePlugin: [pointer]  {}* => [class] TECS::{}Pointer\n".format(
    ...
    def get_type_map_ent(self, ttype):
        # structure type is registered in check_struct_member
    ...
    def gen_cdl_file(self, file):
        ...
        if MrubyBridgeSignaturePluginModule.celltypes.get(self.celltype_name) is None:
            ...
            self.print_msg(
                "  MrubyBridgePlugin: [signature] {} => [celltype] nMruby::{}"
                " => [class] TECS::{}\n".format(
 ...
 *      cell nMruby::{ct} BridgeCellName {{ cTECS = CellName.eEntry; }};
...
""".format(name=name))
            ...
            self.cdl_info(
                "MRBW001 MrubyBridgePlugin: signature '$1' duplicate. ignored current one",
                self.signature.get_namespace_path()
            )
    ...
    def new_cell(self, cell):
        ...
        if MrubyBridgeSignaturePluginModule.VM_list.get(vm_name) is None:
            ...
            # TECS 初期化セル(プロトタイプ宣言)
            self.print_msg(
                "  MrubyBridgePlugin: join your VM's cInitialize to"
...
""".format(vm_name=vm_name))
    ...
    def gen_post_code_body(cls, file):
        ...
        if MrubyBridgeSignaturePluginModule.VM_celltypes is None:
            raise RuntimeError("MrubyBridgeSignaturePlugin: are0")
    ...
    def gen_ep_func_body(self, file, b_singleton, ct_name, global_ct_name,
        ...
        if MrubyBridgeSignaturePluginModule.celltypes.get(ct_name):
            ...
            raise RuntimeError(
                "MrubyBridgeSignaturePlugin: Unknown {}".format(ct_name)
            )

    def gen_ep_func_body_bridge(self, file, b_singleton, ct_name, global_ct_name,
                                sig_name, ep_name, func_name, func_global_name,
                                func_type, params):
        raise RuntimeError("MrubyBridgeSignaturePlugin: unexpected")
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_{ct}_initialize, MRB_ARGS_REQ(1) );
        ...
        for f in self.func_head_array:
            ...
            for param in f.get_paramlist().get_items():
                if param.get_direction() in (Sym("IN"), Sym("INOUT"), Sym("OUT")):
                    ...
                    raise RuntimeError("MrubyBridgeSignaturePlugin: send, receive")
            if n_param > 0:
            ...
            file.print(
                '\tmrb_define_method( mrb, rc, "{fn}", MrubyBridge_{ct}_{fn}, {p_str} );\n'.format(
...
""")
    ...
    def gen_preamble_instance(self, file, b_singleton, ct_name, global_ct_name):
        ...
        nsp = NamespacePath(Sym("nMruby"), True)
...
""".format(ct=self.celltype_name))
        ...
        for cell in ct.get_cell_list():
            if cell.is_generate():
                ...
                join = join_list.get_item(Sym("bridgeName"))
                if join is None:
                    name = '"{}"'.format(cell.get_name())
                else:
                    rhs = join.get_rhs()
...
""")
    ...
    def gen_preamble_instance_proto(self, file, b_singleton, ct_name, global_ct_name):
        file.print(
            "//  Prototype MBP400\n"
            "mrb_value  MrubyBridge_{ct}_initialize("
        ...
        for f in self.func_head_array:
            ...
            file.print(
                "mrb_value  MrubyBridge_{ct}_{fn}("
                " mrb_state *mrb, mrb_value self );\n".format(
...
/* MBP100 */
mrb_value
MrubyBridge_{ct}_initialize( mrb_state *mrb, mrb_value self)
{{
\tmrb_value\tname;
\tstruct tecs_{ct} *tecs_cb;
\tconst struct name_to_cbp_{ct} *ntc;
...
\t}}
\tfor( ntc = &Name_to_cbp_{ct}[0]; ntc->name != NULL; ntc++ ){{
...
\t}}
\ttecs_cb = (struct tecs_{ct} *)mrb_malloc(mrb, sizeof(struct tecs_{ct}) );
\ttecs_cb->cbp = ntc->cbp;
...
mrb_value
MrubyBridge_{ct}_{fn}( mrb_state *mrb, mrb_value self )
{{
\t/* cellcbp (MBP105) */
\tCELLCB\t*p_cellcb = ((struct tecs_{ct} *)DATA_PTR(self))->cbp;
""".format(ct=ct_name, fn=f_func_name))
            ...
            for param in plist:
                ...
                if isinstance(type_, IntType):
                    ...
                    raise RuntimeError("MrubyBridgeSignaturePlugin: Unknown type")
                ...
                for param in plist:
                    ...
                    if isinstance(type_, IntType):
                        ...
                        if isinstance(ttype, StructType):
                            ...
                            raise RuntimeError(
                                "MrubyBridgeSignaturePlugin: cannot handle type"
                            )
                    elif isinstance(type_, StructType):
                        file.print(
                            "\tCHECK_STRUCT( {}, mrb_{} );\n".format(
                        ...
                        raise RuntimeError(
                            "MrubyBridgeSignaturePlugin: canot treat class"
                        )
