...
void *tTask_call_cTaskBody_main( void *p )
{
	...
	fprintf( stderr, "*** starting task '%s'  %08lX\n", ATTR_name, (unsigned long)tTask_call_cTaskBody_main );
	cTaskBody_main();
...
int main(int arg, char **argv)
{
	...
	FOREACH_CELL( i, p_cellcb )
		...
		pthread_create( &VAR_my_thread, NULL, tTask_call_cTaskBody_main, (void *)p_cellcb );
