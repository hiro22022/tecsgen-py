/*
 * This file was automatically generated by tecsgen.
 * Move and rename like below before editing,
 *   gen/tCell2_templ.c => src/tCell2.c
 * to avoid to be overwritten by tecsgen.
 */
/* #[<PREAMBLE>]#
 * Don't edit the comments between #[<...>]# and #[</...>]#
 * These comment are used by tecsmerege when merging.
 *
 * attr access macro #_CAAM_#
 * a                int32_t          ATTR_a          
 *
 * call port function #_TCPF_#
 * call port: cCall2 signature: sSig3 context:task
 *   int32_t        cCall2_func1( subscript, int32_t a );
 *   int32_t        cCall2_func2( subscript, int32_t a );
 *       subscript:  0...(NCP_cCall2-1)
 *
 * #[</PREAMBLE>]# */

/* Put prototype declaration and/or variale definition here #_PAC_# */
#include "tCell2_tecsgen.h"

#ifndef E_OK
#define	E_OK	0		/* success */
#define	E_ID	(-18)	/* illegal ID */
#endif

/* entry port function #_TEPF_# */
/* #[<ENTRY_PORT>]# eEntry2
 * entry port: eEntry2
 * signature:  sSig2
 * context:    task
 * #[</ENTRY_PORT>]# */

/* #[<ENTRY_FUNC>]# eEntry2_func1
 * name:         eEntry2_func1
 * global_name:  tCell2_eEntry2_func1
 * oneway:       false
 * #[</ENTRY_FUNC>]# */
int32_t
eEntry2_func1(CELLIDX idx, int32_t a)
{
	CELLCB	*p_cellcb = GET_CELLCB(idx);

	/* Put statements here #_TEFB_# */
#warning "'eEntry2_func1' needs to be edited."   /* delete this line after edit */

}

/* #[<ENTRY_FUNC>]# eEntry2_func2
 * name:         eEntry2_func2
 * global_name:  tCell2_eEntry2_func2
 * oneway:       false
 * #[</ENTRY_FUNC>]# */
int32_t
eEntry2_func2(CELLIDX idx, int32_t a)
{
	CELLCB	*p_cellcb = GET_CELLCB(idx);

	/* Put statements here #_TEFB_# */
#warning "'eEntry2_func2' needs to be edited."   /* delete this line after edit */

}

/* #[<ENTRY_PORT>]# eEntryB
 * entry port: eEntryB
 * signature:  sSigB
 * context:    task
 * #[</ENTRY_PORT>]# */

/* #[<ENTRY_FUNC>]# eEntryB_func1
 * name:         eEntryB_func1
 * global_name:  tCell2_eEntryB_func1
 * oneway:       false
 * #[</ENTRY_FUNC>]# */
int32_t
eEntryB_func1(CELLIDX idx, int32_t a)
{
	CELLCB	*p_cellcb = GET_CELLCB(idx);

	/* Put statements here #_TEFB_# */
#warning "'eEntryB_func1' needs to be edited."   /* delete this line after edit */

}

/* #[<ENTRY_FUNC>]# eEntryB_func2
 * name:         eEntryB_func2
 * global_name:  tCell2_eEntryB_func2
 * oneway:       false
 * #[</ENTRY_FUNC>]# */
int32_t
eEntryB_func2(CELLIDX idx, int32_t a)
{
	CELLCB	*p_cellcb = GET_CELLCB(idx);

	/* Put statements here #_TEFB_# */
#warning "'eEntryB_func2' needs to be edited."   /* delete this line after edit */

}

/* #[<POSTAMBLE>]#
 *   Put non-entry functions below.
 * #[</POSTAMBLE>]#*/
